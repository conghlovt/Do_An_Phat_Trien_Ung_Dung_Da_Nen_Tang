...
export class HotelService {
        ...
        const hotel = await prisma.hotel.create({
            data: {
                ...
                ...(data.amenityIds && data.amenityIds.length > 0
                    ...
                    : {}),
            }, // <--- Thêm "as any" vào đây để xóa bỏ gạch đỏ TypeScript
     ...
     */
    async update(hotelId, ownerId, data) {
            ...
            if (data.address) {
                ...
                if (currentAddr) {
                    ...
                    await tx.hotelAddress.update({
                        where: { hotelId },
                        data: { ...data.address, fullAddress }, // Ép kiểu chống lỗi strict type của Prisma 7.8
                    });
                }
            }
            ...
            if (data.amenityIds) {
                ...
                if (data.amenityIds.length > 0) {
                    await tx.hotelAmenity.createMany({
                        data: data.amenityIds.map((amenityId) => ({ hotelId, amenityId })), // Ép kiểu chống lỗi mapping mảng dữ liệu
                    });
                }
            }
    ...
    async getById(hotelId, ownerId) {
        ...
        if (!hotel)
            throw new NotFoundError('Không tìm thấy khách sạn', 'HOTEL_NOT_FOUND');
        if (hotel.ownerId !== ownerId)
            throw new ForbiddenError('Bạn không có quyền xem khách sạn này');
        return hotel;
    }
    /**
     ...
     */
    async submitForReview(hotelId, ownerId) {
        const hotel = await this.verifyOwnership(hotelId, ownerId);
        if (hotel.status !== 'draft' && hotel.status !== 'rejected') {
            throw new UnprocessableError('Chỉ có thể gửi duyệt khách sạn ở trạng thái nháp hoặc bị từ chối', 'HOTEL_INVALID_STATUS');
        }
     ...
     */
    async delete(hotelId, ownerId) {
        const hotel = await this.verifyOwnership(hotelId, ownerId);
        if (hotel.status !== 'draft' && hotel.status !== 'rejected') {
            throw new UnprocessableError('Chỉ có thể xóa khách sạn ở trạng thái nháp hoặc bị từ chối', 'HOTEL_CANNOT_DELETE');
        }
     ...
     */
    async addImages(hotelId, ownerId, files) {
        await this.verifyOwnership(hotelId, ownerId);
        const uploadedFiles = await uploadService.uploadMultiple(files, BUCKETS.HOTEL_IMAGES, 'hotel', hotelId, ownerId);
     ...
     */
    async removeImage(hotelId, imageId, ownerId) {
        await this.verifyOwnership(hotelId, ownerId);
        const image = await prisma.hotelImage.findFirst({
            where: { id: imageId, hotelId },
        });
        if (!image)
            throw new NotFoundError('Không tìm thấy ảnh', 'IMAGE_NOT_FOUND');
     ...
     */
    async addVideo(hotelId, ownerId, file, title) {
        await this.verifyOwnership(hotelId, ownerId);
        const uploadedFile = await uploadService.uploadFile(file, BUCKETS.HOTEL_VIDEOS, 'hotel', hotelId, ownerId);
     ...
     */
    async removeVideo(hotelId, videoId, ownerId) {
        await this.verifyOwnership(hotelId, ownerId);
        const video = await prisma.hotelVideo.findFirst({
            where: { id: videoId, hotelId },
        });
        if (!video)
            throw new NotFoundError('Không tìm thấy video', 'VIDEO_NOT_FOUND');
        if (video.fileId) {
