...
export class InventoryService {
     ...
     */
    async getCalendar(hotelId, ownerId, startDate, endDate) {
        // Verify ownership
     ...
     */
    async updateInventory(hotelId, roomTypeId, ownerId, date, data) {
        // Verify ownership
        const hotel = await prisma.hotel.findUnique({
            where: { id: hotelId },
            select: { id: true, ownerId: true },
        ...
        if (!hotel)
            throw new NotFoundError('Không tìm thấy khách sạn', 'HOTEL_NOT_FOUND');
        if (hotel.ownerId !== ownerId)
            throw new ForbiddenError('Bạn không có quyền chỉnh sửa');
