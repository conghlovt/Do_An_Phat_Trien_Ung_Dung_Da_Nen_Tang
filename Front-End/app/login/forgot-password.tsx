...
import { forgotPassword, resetPassword } from '../../src/login/api/auth.api';
...
export default function ForgotPasswordScreen() {
  ...
  const onForgotSubmit = async (data: ForgotFormData) => {
    ...
    try {
      ...
      setStep(2);
      Alert.alert('Thành công', 'Mã đặt lại mật khẩu đã được gửi đến email của bạn.');
    } catch (err) {
      setError(getApiErrorMessage(err, 'Không thể gửi mã đặt lại mật khẩu. Vui lòng thử lại.'));
    } finally {
      setIsLoading(false);
    }
  };
