...
export function DepositPolicy({ onBack }: Props) {
  ...
  useEffect(() => {
    if (currentHotel) setSelectedPercent(currentHotel.depositPercent ?? 0);
  }, [currentHotel]);
  ...
  const handleSave = async () => {
    try {
      ...
      setSuccessMsg('Đã cập nhật chính sách đặt cọc thành công!');
      setTimeout(() => {
        setSuccessMsg('');
        onBack?.();
      }, 1500);
    ...
    <View style={s.container}>
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
        <View style={s.headerCard}>
          <View style={s.headerIconWrapper}><Wallet size={28} color="#1D4ED8" /></View>
...
const s = StyleSheet.create({
  ...
  headerCard: { marginHorizontal: 20, marginTop: 20, backgroundColor: '#EFF6FF', borderRadius: 16, padding: 24, alignItems: 'center', borderWidth: 1, borderColor: '#BFDBFE' },
  ...
  infoCard: { marginHorizontal: 20, marginTop: 20, backgroundColor: '#EFF6FF', borderRadius: 10, padding: 14, flexDirection: 'row', gap: 10, borderWidth: 1, borderColor: '#BFDBFE' },
  infoText: { flex: 1, fontSize: 12, color: '#1E40AF', lineHeight: 18 },
  actions: { marginHorizontal: isMobile ? 16 : 20, marginTop: isMobile ? 24 : 32 },
  saveBtn: { backgroundColor: '#1D4ED8', paddingVertical: 14, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  ...
  successBox: { marginHorizontal: 20, marginTop: 16, backgroundColor: '#EFF6FF', padding: 12, borderRadius: 8, borderWidth: 1, borderColor: '#BFDBFE' },
  successText: { color: '#1D4ED8', fontSize: 14, textAlign: 'center' },
});
