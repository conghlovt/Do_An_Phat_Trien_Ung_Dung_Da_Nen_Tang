...
const MetricCard = ({ title, value, trend, icon: Icon, color }: any) => {
  ...
  return (
    <View style={[styles.metricCard, !isLight && styles.surfaceDark]}>
      ...
      <Text style={[styles.metricValue, !isLight && styles.textLight]}>{value}</Text>
...
export const DashboardOverview = () => {
  ...
  if (loading) {
    return <View style={styles.container}><Text style={{color: isLight ? '#0F172A' : '#FFFFFF'}}>Đang tải dữ liệu...</Text></View>;
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.chartGrid}>
        ...
        <View style={[styles.chartCard, !isLight && styles.surfaceDark, { flex: 2 }]}>
          <Text style={[styles.chartTitle, !isLight && styles.textLight]}>Doanh thu & Booking theo ngày</Text>
              ...
              <AreaChart data={REVENUE_DATA}>
                ...
                <Tooltip 
                  contentStyle={{
                    ...
                    boxShadow: isLight ? '0 10px 24px rgba(15, 23, 42, 0.12)' : 'none',
                  }}
        ...
        <View style={[styles.chartCard, !isLight && styles.surfaceDark, { flex: 1 }]}>
          <Text style={[styles.chartTitle, !isLight && styles.textLight]}>Trạng thái Booking</Text>
          <View style={{ height: 300, width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
      ...
      <View style={styles.activityGrid}>
        <View style={[styles.activityCard, !isLight && styles.surfaceDark]}>
          <Text style={[styles.chartTitle, !isLight && styles.textLight]}>Hoạt động mới nhất</Text>
          {[1, 2, 3, 4, 5].map((i) => (
            <View key={i} style={[styles.activityItem, !isLight && styles.activityItemDark, i === 5 && { borderBottomWidth: 0 }]}>
              <View style={styles.avatarMini}>
                <Users size={18} color="#3B82F6" />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.activityName, !isLight && styles.textLight]}>Khách hàng đăng ký #{i}029</Text>
                <Text style={styles.activityDetail}>Vừa tham gia hệ thống từ Hà Nội</Text>
              </View>
              <View style={styles.activityStatus}>
                <Text style={[styles.timeText, !isLight && styles.mutedTextDark]}>{i * 2} phút trước</Text>
              </View>
            </View>
          ))}
        </View>
        ...
        <View style={[styles.activityCard, !isLight && styles.surfaceDark]}>
          <Text style={[styles.chartTitle, !isLight && styles.textLight]}>Yêu cầu đối tác</Text>
          {[1, 2, 3, 4, 5].map((i) => (
            <View key={i} style={[styles.activityItem, !isLight && styles.activityItemDark, i === 5 && { borderBottomWidth: 0 }]}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.activityName, !isLight && styles.textLight]}>Khách sạn ABC Luxury</Text>
...
const styles = StyleSheet.create({
  ...
  metricCard: {
    ...
    borderColor: '#E2E8F0',
    ...Platform.select({
      web: { boxShadow: '0 10px 24px rgba(15, 23, 42, 0.08)' } as any,
      default: {
        shadowColor: '#0F172A',
