...
export const ContentManagement = ({ permissions = fullAccess }: { permissions?: ModuleAccess }) => {
  ...
  const columns = [
    { key: 'title', label: 'Tiêu đề', render: (value: string) => <Text style={{ color: isLight ? '#0F172A' : '#FFFFFF', fontWeight: '700' }}>{value}</Text> },
    { key: 'category', label: 'Danh mục' },
    { key: 'author', label: 'Người viết' },
    { key: 'status', label: 'Trạng thái', render: (s: string) => <Text style={{ color: s === 'PUBLISHED' ? (isLight ? '#047857' : '#10B981') : (isLight ? '#64748B' : '#94A3B8') }}>{s === 'PUBLISHED' ? 'Đã đăng' : s === 'DRAFT' ? 'Bản nháp' : 'Lưu trữ'}</Text> },
    { key: 'date', label: 'Cập nhật cuối', render: (date: string) => <Text style={{ color: isLight ? '#64748B' : '#94A3B8' }}>{new Date(date).toLocaleDateString('vi-VN')}</Text> },
  ];
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TextInput style={styles.input} placeholder="VD: Top 10 địa điểm du lịch..." placeholderTextColor="#94A3B8" value={formData.title} onChangeText={(title) => setFormData({ ...formData, title })} />

              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>Danh mục</Text>
                  <TextInput style={styles.input} placeholder="VD: Cẩm nang" placeholderTextColor="#94A3B8" value={formData.category} onChangeText={(category) => setFormData({ ...formData, category })} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>Ảnh đại diện (URL)</Text>
                  <TextInput style={styles.input} placeholder="https://..." placeholderTextColor="#94A3B8" value={formData.thumbnail} onChangeText={(thumbnail) => setFormData({ ...formData, thumbnail })} />
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  modalContent: { backgroundColor: '#FFFFFF', width: '100%', maxWidth: 720, maxHeight: '90%', borderRadius: 24, padding: 24, borderWidth: 1, borderColor: '#E2E8F0', ...Platform.select({ web: { boxShadow: '0 18px 36px rgba(15, 23, 42, 0.16)' } as any }) },
