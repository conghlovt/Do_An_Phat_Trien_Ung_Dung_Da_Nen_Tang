...
const toProvince = (location: LocationProvince): Province => {
  const knownProvince = matchProvince(location.name);
  const normalizedName = normalizeKey(location.name);

  return {
    id: knownProvince?.id ?? normalizedName,
    name: knownProvince?.name ?? location.name,
    code: knownProvince?.code ?? location.name,
    region: knownProvince?.region ?? '',
    numberOfHotels: location.count,
...
export function LocationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [availableProvinces, setAvailableProvinces] = useState<Province[]>([PROVINCES[0]]);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    let cancelled = false;

    locationsApi.getCustomerLocations()
      .then((locations) => {
        if (cancelled) return;

        const provincesFromApi = locations
          .map(toProvince)
          .sort((a, b) => compareVietnamese(a.name, b.name));

        if (provincesFromApi.length === 0) return;

        setAvailableProvinces(provincesFromApi);
        setSelectedProvince((current) => {
          const matched = provincesFromApi.find(
            (province) => province.id === current.id || normalizeKey(province.name) === normalizeKey(current.name),
          );

          return matched ?? provincesFromApi[0] ?? current;
        });
      })
      .catch((err) => {
  ...
  const detectLocation = useCallback(async () => {
    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        if (matched) {
          const availableMatched = availableProvinces.find(
            (province) => province.id === matched.id || normalizeKey(province.name) === normalizeKey(matched.name),
          );

          setSelectedProvince(availableMatched ?? matched);
          return;
        }
      }
