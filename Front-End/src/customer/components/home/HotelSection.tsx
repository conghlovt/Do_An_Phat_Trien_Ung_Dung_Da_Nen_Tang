...
export default function HotelSection({ title, hasMore, tabs }: HotelSectionProps) {
  ...
  const handleSeeAll = () => {
    router.push({ pathname: '/customer/hotels', params: { section: title, tab: activeTab } });
  };
          ...
          {loading
            ...
            : error
              ...
              : hotels.length === 0
                ...
                : hotels.slice(0, SECTION_PREVIEW_LIMIT).map((hotel) => <HotelCard key={hotel.id} {...hotel} id={hotel.id} bookingType={activeTab} />)
          }
        </View>
      ) : (
        <ScrollView
